...
def archive_result(
    ...
    # Find the prompt in the slot to get its output_id
    slot_prompts = image_slots[slot_index].get("prompts", [])
    prompt = next((p for p in slot_prompts if p["id"] == prompt_id), None)
    if not prompt:
        raise KeyError(f"Prompt '{prompt_id}' not found in slot {slot_index}")

    oid = prompt.get("output_id")
    if not oid:
        raise ValueError(f"Prompt '{prompt_id}' nie ma output_id — nie można znaleźć pliku")
    ...
    if src is None:
        raise FileNotFoundError(
            f"Output file not found for prompt '{prompt_id}' "
            f"(slot {slot_index}) in {output_dir}"
        )
...
async def _run_tile_async(
    ...
    try:
        ...
        source_dir    = Path(tile.get("source_dir",   ""))
        output_dir    = Path(tile.get("output_dir",   ""))
        global_suffix = tile.get("global_suffix",      "")
        tile_slug     = _slugify(tile.get("name", tile_id))
        image_slots   = tile.get("image_slots", [])
        ...
        for slot in slots_to_run:
            ...
            if not source_path.exists():
                _log(f"  ⚠ Obrazek nie istnieje, pomijam: {source_path}")
                continue
...
def start_tile_run(
    ...
    asyncio.create_task(_run_tile_async(run_id, tile_id, force_all, slot_index))
    return "ok"
