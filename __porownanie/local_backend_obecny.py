...
class LocalBackend(BaseBackend):
    ...
    def execute(self, inputs, params, workflow):
        ...
        try:
            ...
            # Start frame → start_image in WAN (FIRST frame)
            ...
            # End frame → end_image in WAN (LAST frame - target for interpolation)
            # ============================================================
            # I2V mode: Skip end_image (free generation without constraint)
            # I2V2I mode: Set end_image (interpolate start→end)
            # ============================================================
            
            if is_i2v_mode:
                self.logger.info(f"  Skipping end_image (I2V mode - no interpolation constraint)")
            else:
                # I2V2I mode - set end frame for interpolation
                if not self.workflow_runner.set_image(
