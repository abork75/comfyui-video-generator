...
class LocalBackend(BaseBackend):
    ...
    def execute(self, inputs, params, workflow):
        ...
        try:
            # ============================================================
            # DETECT MODE: I2V (chain) vs I2V2I (interpolation)
            # ============================================================
            
            is_i2v_mode = params.get('is_i2v_mode', False) or inputs['end_frame'] is None
            
            if is_i2v_mode:
                self.logger.info(f"  Mode: I2V (free generation - chain step)")
            else:
                self.logger.info(f"  Mode: I2V2I (interpolation - normal transition)")
            ...
            # ============================================================
            # CRITICAL FIX: Disconnect end_image in I2V mode
            # Node WanFirstLastFrameToVideo requires either:
            # - Both start_image AND end_image (I2V2I interpolation)
            # - Only start_image (I2V free generation)
            # We remove end_image connection from workflow for I2V mode
            # ============================================================
            
            if is_i2v_mode:
                self.logger.info(f"  I2V mode: Disconnecting end_image from workflow")
                
                # Find WAN node (WanFirstLastFrameToVideo)
                wan_node_id = '67'
                ...
                if wan_node_id in self.workflow_runner.workflow:
                    ...
                    if 'end_image' in wan_inputs:
                        # Remove end_image connection
                        del wan_inputs['end_image']
                        self.logger.info(f"    ✓ Removed end_image from Node {wan_node_id}")
            ...
            # End frame - ONLY in I2V2I mode!
            if not is_i2v_mode:
                # I2V2I mode - set different end frame for interpolation
                if not self.workflow_runner.set_image(
                ...
                # I2V mode - skip (already disconnected above!)
                self.logger.info(f"  I2V mode: Skipping end_image upload (not needed)")
