...
class LinuxBackend(BaseBackend):
    ...
    def execute(self, inputs, params, workflow):
        ...
        try:
            ...
            # ============================================================
            # I2V MODE: Odłącz end_image z node WanVideoImageToVideoEncode
            # Node 89 w tym workflow
            # W trybie I2V end_image musi być usunięty z inputs
            # ============================================================

            if is_i2v_mode:
                self.logger.info(f"  I2V mode: Odłączam end_image z workflow")
                wan_node_id = '89'
                ...
                if wan_node_id in self.workflow_runner.workflow:
                    ...
                    if 'end_image' in wan_inputs:
                        ...
                        self.logger.info(f"    ✓ end_image nie było w Node {wan_node_id} (OK)")

            # ============================================================
            # 1. Ustaw obraz startowy
            # ============================================================
            ...
            if not is_i2v_mode:
                ...
                # Upload pliku przez API
                uploaded_name = self.workflow_runner.upload_image(str(inputs['end_frame']))
                if not uploaded_name:
                    self.logger.error(f"  Błąd uploadu end_image")
                    return None

                # Dodaj dynamiczny LoadImage node dla end_image
                # Node 89 (WanVideoImageToVideoEncode) oczekuje tensora obrazu,
                # nie stringa - musimy podłączyć przez LoadImage tak jak start_image
                end_loader_node_id = '9999'
                self.workflow_runner.workflow[end_loader_node_id] = {
                    "inputs": {"image": uploaded_name},
                    "class_type": "LoadImage",
                    "_meta": {"title": "End frame (dynamic)"}
                }

                # Podłącz node 9999 do node 89 jako end_image (output index 0)
                wan_encode_node = '89'
                self.workflow_runner.workflow[wan_encode_node]['inputs']['end_image'] = [end_loader_node_id, 0]
                self.logger.success(f"  OK end_image -> Node {end_loader_node_id} (LoadImage) -> Node {wan_encode_node}.end_image")
            else:
